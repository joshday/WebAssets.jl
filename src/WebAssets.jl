...
"""
add(url; force::Bool=false) = add(MODULE[], url; force)

function add(m::Module, url; force::Bool=false)
    file = url2path(url, dir(m))
    isfile(file) && !force ? file : download(url, file)
end
...
Info(path::AbstractString) = Info(path2url(path), unix2datetime(mtime(path)), filesize(path))
